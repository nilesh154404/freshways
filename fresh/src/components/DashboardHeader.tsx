    ...
    const fetchData = async () => {
      try {
        ...
        const [vendorRes, categoryRes] = await Promise.all([
          axios.get<VendorProfile>(`http://localhost:3064/vendors/${profileId}`, {
            headers: authHeaders,
          }),
          axios.get<Category[]>("http://localhost:3064/categories/get-categories"),
        ]);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const res = await axios.patch<VendorProfile>(
        `http://localhost:3064/vendors/${profileId}/profile`,
        payload,
        { headers: authHeaders }
      );
